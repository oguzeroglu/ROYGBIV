...
MeshGenerator.prototype.generateInstancedMesh = function(graphicsGroup, objectGroup){
  ...
  if (objectGroup.hasEmissiveMap()){
    if (!objectGroup.skipTotalEmissiveColorUniform){
  ...
  if (objectGroup.hasDisplacementMap() && VERTEX_SHADER_TEXTURE_FETCH_SUPPORTED && !objectGroup.skipTotalDisplacementInfoUniform){
...
MeshGenerator.prototype.generateSkybox = function(skybox, isMock){
  ...
  var material = new THREE.RawShaderMaterial({
    ...
    uniforms: {
      ...
      selectiveBloomFlag: new THREE.Uniform(0)
    }
  });
  var mesh = new THREE.Mesh(this.geometry, material);
