    ...
    try{
      ...
      switch (commandIndex){
        ...
        case 239: //newJumpDescriptor
          ...
          var takeoffPoint = markedPoints[splitted[2]];
          var landingPoint = markedPoints[splitted[3]];

          if (!takeoffPoint){
            terminal.printError(Text.TAKEOFF_POINT_DOES_NOT_EXIST);
            return true;
          }

          if (!landingPoint){
            terminal.printError(Text.LANDING_POINT_DOES_NOT_EXIST);
            return true;
          }

          if (takeoffPoint.registeredSceneName != sceneHandler.getActiveSceneName()){
            terminal.printError(Text.TAKEOFFPOINT_IS_NOT_IN_THE_ACTIVE_SCENE);
            return true;
          }

          if (landingPoint.registeredSceneName != sceneHandler.getActiveSceneName()){
            terminal.printError(Text.LANDING_POINT_IS_NOT_IN_THE_ACTIVE_SCENE);
            return true;
          }

          if (splitted[2] == splitted[3]){
            terminal.printError(Text.TAKEOFF_POINT_AND_LANDING_POINT_ARE_THE_SAME);
            return true;
          }

          var runupSatisfactionRadius = parseFloat(splitted[4]);
          var takeoffPositionSatisfactionRadius = parseFloat(splitted[5]);
          var takeoffVelocitySatisfactionRadius = parseFloat(splitted[6]);

          if (isNaN(runupSatisfactionRadius)){
            terminal.printError(Text.IS_NOT_A_NUMBER.replace(Text.PARAM1, "runupSatisfactionRadius"));
            return true;
          }

          if (isNaN(takeoffPositionSatisfactionRadius)){
            terminal.printError(Text.IS_NOT_A_NUMBER.replace(Text.PARAM1, "takeoffPositionSatisfactionRadius"));
            return true;
          }

          if (isNaN(takeoffVelocitySatisfactionRadius)){
            terminal.printError(Text.IS_NOT_A_NUMBER.replace(Text.PARAM1, "takeoffVelocitySatisfactionRadius"));
            return true;
          }

          if (runupSatisfactionRadius < 0){
            runupSatisfactionRadius = 0;
          }

          if (takeoffPositionSatisfactionRadius < 0){
            takeoffPositionSatisfactionRadius = 0;
          }

          if (takeoffVelocitySatisfactionRadius < 0){
            takeoffVelocitySatisfactionRadius = 0;
          }

          steeringHandler.addJumpDescriptor(id, takeoffPoint, landingPoint, runupSatisfactionRadius, takeoffPositionSatisfactionRadius, takeoffVelocitySatisfactionRadius);
