var Text = function(){
 ...
 this.ROYGBIV_SCRIPTING_API_CREATEFPSCONTROL = "Creates a new FPSControls object to be used in First Person Shooter games for both desktop and mobile devices. FPSControls automatically\nhandles the PointerLock as well. The controls are:\n"+
  ...
  this.ROYGBIV_SCRIPTING_API_DISCONNECTFROMSERVER = "Disconnects from server also clears all networking related callbacks. Does nothing if not connected to server.";
  this.ROYGBIV_SCRIPTING_API_ONDISCONNECTEDFROMSERVER = "Sets a listener for server connection status. The callbackFunction is executed when the connection between the server and the client is lost.";
}
